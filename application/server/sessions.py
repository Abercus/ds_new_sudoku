...
class gameSession:
		...
		if self.started:	#if game ongoing, send info to this person
			...
			if len(self.subs)>=self.prefplayers:
				...
				for sub in self.subs:
					send_gstate(sub) #sends opening board to everyone
			else:
		...
		if len(self.subs) == 1:
			...
			for pl in self.ldboard:
				if self.ldboard[pl]>winsc:
					...
					winpl=pl
			res=PUSH_SESS_ENDED+MSG_FIELD_SEP+winpl #send winner!
	...
	def process(self,message):
		...
		if payload.startswith(REQ_GUESS): #user makes guess
			...
			if info.boardstate[x][y]!="-": #no guess possible here
				pass
			elif self.board[1][x][y]!=g:
				self.ldboard[user]-=1
				#wrong guess
				res=PUSH_SESS_UPDATE+MSG_FIELD_SEP+"0"+str(x)+str(y)+str(g)+MSG_SEP+str(self.ldboard)[1:-2]
				for sub in self.subs:
					sub.notify(res)
			else:
				self.ldboard[user]+=1
				self.boardstate[x][y]=g
				res=PUSH_SESS_UPDATE+MSG_FIELD_SEP+"1"+str(x)+str(y)+str(g)+MSG_SEP+str(self.ldboard)[1:-2]
				for sub in self.subs:
					sub.notify(res)
